...
cd /d "%PROJECT_DIR%"
python -m nuitka --standalone --output-dir=ft1-backend-dist --windows-console-mode=disable --jobs=0 --enable-plugin=numpy --enable-plugin=scipy --include-package=backend --include-package=fastapi --include-package=uvicorn --include-package=sqlalchemy --include-package=pydantic --include-package=statsmodels --include-package=pymssql --include-package=apscheduler --include-package=access_parser --include-package=pydantic_settings --include-package=multipart --include-package=websockets --include-package=yaml --include-package=pyodbc --include-package=sklearn --include-package=pandas --include-package=starlette --include-package=scipy._external --include-module=ctypes --include-data-dir=data=data --nofollow-import-to=scipy.io,scipy.cluster --nofollow-import-to=numpy.tests --nofollow-import-to=pandas.conftest backend/run.py || (echo 后端编译失败 & pause & exit /b 1)
...
REM 复制配置文件
for %%f in (instrument_config.json prediction_config.json db_mapping.json product_categories.json product_indicators.json product_status.json excluded_remarks.json alias_config.json alert_rules.json fta_config.json spec_limits.json db_config.json mdb_config.json) do copy /Y "!PROJECT_DIR!%%f" "%BACKEND_DIST%\" >nul 2>nul
