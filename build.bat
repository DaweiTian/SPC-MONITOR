...
cd /d "%PROJECT_DIR%"
python -m nuitka --standalone --output-dir=ft1-backend-dist --windows-console-mode=disable --jobs=0 --enable-plugin=numpy --enable-plugin=scipy --include-package=backend --include-package=fastapi --include-package=uvicorn --include-package=sqlalchemy --include-package=pydantic --include-package=statsmodels --include-package=pymssql --include-package=apscheduler --include-package=access_parser --include-package=pydantic_settings --include-package=multipart --include-package=websockets --include-package=yaml --include-package=pyodbc --include-package=sklearn --include-package=pandas --include-package=starlette --include-module=ctypes --include-data-dir=data=data --noinclude-pytest-mode=nofollow --noinclude-setuptools-mode=nofollow --nofollow-import-to=scipy.io,scipy.weave,scipy.cluster --nofollow-import-to=numpy.tests --nofollow-import-to=sklearn.datasets,sklearn.tests backend/run.py || (echo 后端编译失败 & pause & exit /b 1)
