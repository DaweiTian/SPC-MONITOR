...
cd /d "%PROJECT_DIR%"
python -m nuitka --standalone --output-dir=ft1-backend-dist --windows-console-mode=disable --jobs=0 --include-package=backend --include-package=fastapi --include-package=uvicorn --include-package=sqlalchemy --include-package=pydantic --include-package=statsmodels --include-package=pymssql --include-package=apscheduler --include-package=access_parser --include-package=pydantic_settings --include-package=python_multipart --include-package=websockets --include-package=yaml --include-package=pyodbc --include-package=scipy._external --include-package=scipy._external.array_api_compat --include-package=scipy._external.array_api_compat.numpy --include-package=scipy._lib --include-package=numpy._core --include-package=numpy.fft --include-package=numpy.random --include-package=numpy.linalg --include-data-dir=data=data --nofollow-import-to=tkinter --nofollow-import-to=matplotlib --nofollow-import-to=PIL --nofollow-import-to=pytest --nofollow-import-to=unittest backend/run.py || (echo 后端编译失败 & pause & exit /b 1)
