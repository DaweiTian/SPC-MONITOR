...
cd /d "%PROJECT_DIR%"
python -m nuitka --standalone --output-dir=ft1-backend-dist --windows-console-mode=disable --jobs=0 --include-package=backend --include-package=fastapi --include-package=uvicorn --include-package=sqlalchemy --include-package=pydantic --include-package=scipy.stats --include-package=scipy.special --include-package=scipy._lib --include-package=pymssql --include-package=apscheduler --include-package=access_parser --include-package=pydantic_settings --include-package=python_multipart --include-package=websockets --include-package=yaml --include-package=pyodbc --include-package=numpy --include-data-dir=data=data --noinclude-pytest-mode=nofollow --noinclude-setuptools-mode=nofollow backend/run.py || (echo 后端编译失败 & pause & exit /b 1)
