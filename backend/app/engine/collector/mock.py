...
MOCK_INDICATORS = [
    {'code': 'fat', 'name': '脂肪', 'unit': 'g/100g', 'mean': 3.8, 'std': 0.15, 'lsl': 3.0, 'usl': 5.0},
    {'code': 'protein', 'name': '蛋白质', 'unit': 'g/100g', 'mean': 3.3, 'std': 0.1, 'lsl': 2.9, 'usl': 3.6},
    {'code': 'ts', 'name': '全脂乳固体', 'unit': 'g/100g', 'mean': 12.5, 'std': 0.3, 'lsl': 11.5, 'usl': 13.0},
...
class MockCollector(BaseCollector):
    ...
    def collect(self) -> Dict[str, Any]:
        ...
        for _ in range(num_records):
            ...
            raw_value = random.gauss(indicator['mean'], indicator['std'])

            if random.random() < 0.05:
                if random.random() < 0.5:
                    raw_value = indicator['usl'] + random.uniform(0.1, 0.5)
                else:
                    raw_value = indicator['lsl'] - random.uniform(0.1, 0.5)

            # Get correction value
            correction = self._get_correction(product['code'], indicator['code'])
