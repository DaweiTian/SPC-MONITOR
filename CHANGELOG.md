...
- **发版前十轮审查修复**: snapshot 指标仅补缺不覆盖用户勾选；采集预警跳过停用品项与未勾选指标；规格限 API 路径编码闭环；预警→SPC 深链 query 编码；配置保存 success 检查与失败保留弹窗；Alerts 分页 total 与可见性对齐；SPC/Capability/Prediction 指标纠偏遵守勾选
- **P2 项一并修复**: 预警列表/角标/看板服务端过滤停用与空勾选品项；SPC/预测/看板等 metadata 就绪后再初始化，减少闪现；数据管理指标下拉与「全部」列表遵守勾选；监测指标数与品项管理一致；别名/修正值配置 JSON 结构防护；删除死代码 ProductManager；品项弹窗关闭时重置状态
